	...
	class PieRegister extends PieReg_Base{
		...
		*/
		...
		function piereg_register_scripts(){
			...
			wp_register_script('pie_datepicker_js',plugins_url("/js/datepicker.js",__FILE__),array('jquery'),'2.0',false);
			...
			wp_register_script('pie_alphanum_js',plugins_url("/js/jquery.alphanum.js",__FILE__),array('jquery'),'2.0',false);
			...
			wp_register_style( 'pie_wload_css', plugins_url("/css/jquery.wload.css",__FILE__),false,'1.0', "all" );
		...
		function piereg_backendregister_scripts(){
			...
			wp_register_script('pie_wload_js',plugins_url("/js/jquery.wload.js",__FILE__),'jquery','1.0',false);
		...
		*/
		...
		function add_pie_form_button($context)
		{
			...
			printf('<a href="%s" class="thickbox button" id="add_pie_form" title="%s"><span style="%s" class="wp-media-buttons-icon"></span>%s</a>','#TB_inline?width=480&inlineId=select_pie_form',__("Add Pie Register Form", 'pie-register'),'background: url('.plugins_url('pie-register').'/images/form-icon.png); background-repeat: no-repeat; background-position: left bottom;',__("Add Form","pie-register"));
...
?>
		...
		function AddPanel()
		{
			...
			$pie_page_suffix_1 = add_menu_page( "Pie Register", __('Pie Register',"pie-register") , 'manage_options', 'pie-register',  array($this,'PieRegNewForm'), plugins_url("/images/pr_icon.png",__FILE__), 60 );
			...
			if(	is_plugin_active('pie-register-aweber/pie-register-aweber.php') || is_plugin_active('pie-register-mailchimp/pie-register-mailchimp.php') )
			{
				...
				$pie_page_suffix_16 = add_submenu_page( 'pie-register', 'Bulk Email', __('Bulk Email',"pie-register") , 'manage_options', 'pie-bulk-emails', array($this, 'PieRegBulkEmails'));
		...
		*/
		...
		function Add_payment_option_PaypalStandard($field_as)
		{
			...
			if( $check_payment["enable_paypal"] == 1 && !empty($check_payment['paypal_butt_id']) )
			{
				if( $field_as == 0 )
					...
					$PR_options = '<option value="PaypalStandard" data-img="'.plugins_url("images/PaypalStandard-logo.png",__FILE__).'">'.__("Paypal (Standard)","pie-register").'</option>';
			}
			return $PR_options;
		}
		...
		function Add_payment_option($send_data,$paymeny_gatways) // Only For Paypal
		{
			...
			if($check_payment["enable_paypal"] == 1 && !(empty($check_payment['paypal_butt_id'])) )
			{
				...
				{
					$send_data .= '<option data-img="'.plugins_url('pie-register/images/paypal_std_btn.png').'" value="PaypalStandard">'.(__("Paypal (Standard)","pie-register")).'</option>';
				}
			}
				...
				if( ($_GET['action'] == "current_email_verify" ) && (isset($_GET['key']) && !empty($_GET['key']) ) && (isset($_GET['login']) && !empty($_GET['login']) ))
				{
					...
					if($email_verify_orignal_key == $email_verify_key)
					{
						...
						*/
						...
						$print_message = __('Verficcation link is expired or invalid.','pie-register' );
						$type = "error";
					}
				}
